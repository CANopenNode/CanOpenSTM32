...
parseBaudrate(uint8_t baudrate) {
    switch (baudrate) {
        case 0x01:
            return 125;
        case 0x02:
...
/* Data block for storage, which can be stored to non-volatile memory */
 ...
 */
ODR_t
OD_write_layerSettings(OD_stream_t* stream, const void* buf, OD_size_t count, OD_size_t* countWritten) {
    /* verify arguments */
    ...
    /* Check for correct code in the protection subindex if trying to save layer settings */
